...
\begin{frame}
  \chtitle{复变函数的导数}
  \bch
  复变函数$f(z)$的导数还是定义为$f$的微小改变量和$z$的微小该变量之比：
...
\begin{frame}
  \chtitle{泰勒级数}
  \bch
  把这样的过程进行无限多次，就得到泰勒展开公式：
  $$f(z) = \sum_{n=0}^\infty a_n(z-z_0)^n,$$
  其中$a_n = \frac{f^{(n)}(z_0)}{n!}$.

  \skiplines

  (在数学的严谨性上毫无节操的)物理学家虽然对上述一系列操作表现得十分自信，但仍默默担心一件事情：这个级数有可能是发散的。
...
\begin{frame}
  ...
  \bch
 对圆内任意一点$z$，设$|z-z_0|=s<r$，取介于$s$和$r$之间的正数$q$ ($s<q<r$)，以$q$为半径作一个圆$C_q$: $|z-z_0|=q$，则$f(z)$在$C_q$内部解析，边界上连续。于是
...
\begin{frame}
  ...
  \bch
  \addfig{1}{think2.jpg}
  一个无穷点集$S$的{\bf 聚点}是这样的点：在聚点的任意小邻域内都有$S$里的无数个点。
  \bitem
