...
\begin{frame}
  ...
  \bch
  请一位同学总结一下上一周所学的内容：最大模定理，解析延拓和$\Gamma$函数。
  \ech
\end{frame}


\begin{frame}
\chtitle{本讲内容：级数展开和求留数的技巧汇总}
\bch
\bitem
\item{五个谁都知道的公式}
\item{初级技能：变量替换}
...
\secpage{五个谁都知道的公式}{\wulian Qui sont les 谁s？}

\begin{frame}
  \chtitle{  全平面适用的三个展开公式}
  \bch
...
\begin{frame}
  \chtitle{  {\bf 单位圆内适用}的两个公式(规定$z=0$时$1+z$幅角为零)}
  \bch
...
\begin{frame}
  \chtitle{关于幸运大转盘的幸运大转盘问题}
  \bch
  \addfig{1}{drive.jpg}

  如果每个人被幸运大转盘抽中概率相同，请估算你30次都没有被抽中的概率大概是多少。
  \ech
\end{frame}
...
\secpage{高级技能: 去极点}{化Laurent为Taylor}
...
\begin{frame}
  ...
  &=& \sum_{n=1}^\infty \frac{z^n}{n} - \sum_{n=1}^\infty \frac{z^n}{2^nn} - \ln 2 \newl
    &=& \sum_{n=1}^\infty \frac{1-2^{-n}}{n}z^n - \ln 2   
  \eea
  \ech
\end{frame}
...
\begin{frame}
  ...
  &=& \sum_{n=1}^\infty \frac{1-2^n}{n}\frac{1}{z^n} 
  \eea
  \ech
\end{frame}
